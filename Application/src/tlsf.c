...
void *mem_block = (void *) (volatile uint16_t*) (SDRAM_BANK_ADDR);
...
static size_t adjust_request_size(size_t size, size_t align){
    ...
    if (size){
        ...
        if (aligned < block_size_max) adjust = tlsf_max(aligned, block_size_min);
    }
    return adjust;
}
...
static void control_construct(control_t* control){
    ...
    for (i = 0; i < FL_INDEX_COUNT; ++i){
        ...
        for (j = 0; j < SL_INDEX_COUNT; ++j) control->blocks[i][j] = &control->block_null;
    }
}
...
void tlsf_create(){
    if (((tlsfptr_t)mem_block % ALIGN_SIZE) != 0) return;
    control_construct(tlsf_cast(control_t*, mem_block));
    sdram_tlsf = tlsf_cast(tlsf_t, mem_block);
}
...
pool_t tlsf_get_pool(){
    return tlsf_cast(pool_t, (char*)sdram_tlsf + tlsf_size());
}
...
void* tlsf_realloc(void* ptr, size_t size){
    control_t* control = tlsf_cast(control_t*, sdram_tlsf);
    void* p = 0;
    if (ptr && size == 0) tlsf_free(ptr);
    else if (!ptr) p = tlsf_malloc(size);
    else{
        block_header_t* block = block_from_ptr(ptr);
        block_header_t* next = block_next(block);
